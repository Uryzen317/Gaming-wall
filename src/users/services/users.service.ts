...
export class UsersService {
  ...
  async accountDetails(id: number) {
    ...
    let user : any = await this.users.findOne({_id : id} , {salt : false, password: false}).lean();
  ...
  async deAdmin(id : string){
    return await this.users.findOneAndUpdate({_id : id } , { $set : {role : 'user'} });
  }

  async admin(id : string){
    return await this.users.findOneAndUpdate({_id : id } , { $set : {role : 'admin'} });
  }
  ...
  async checkRecaptcha(token : string){
    ...
    try{
      let validation = await axios.post(`https://www.google.com/recaptcha/api/siteverify?secret=${secret}&response=${token}`, {});
  ...
  async forgotPassword(email : string){
    ...
    await this.tokens.updateMany({data: email, operation: 'forgot-password', isExpired: true}, {$set: {isExpired : false}});
    ...
    await this.mailService.mail(email, 'فراموشی رمز عبور', `بر روی لینک زیر کلیک کنید \n ${this.configService.get("CLIENT_ADDRESS")}#/panel/forgotPassword/${token}`);
    return {}
  }
  ...
  async setForgottonPassword(token : string, password : string){
    ...
    if(tokenExpiry > 15){
      await this.logsService.log('forgot-password failed', isValid.email, isValid.email);
      await this.tokens.updateMany({_id : isValid._id}, {$set: {isExpired : true}});
      throw new BadRequestException({message : 'این کد منقضی شده است'});
    }

    await this.tokens.findOneAndUpdate({_id : isValid.id}, {$set : {isExpired : true}});
  ...
  async getConfirmEmail(accessToken : string){
    ...
    await this.tokens.updateMany({ operation: 'email-confirm', data: user.email, isExpired: false}, {$set: {isExpired: true}});
    //generate new token
    ...
    await this.mailService.mail(user.email, 'تایید ایمیل', `کد تایید ایمیل شما \n ${token}`);
  ...
  async confirmEmail(id: string, key: number){
    ...
    // token validation
    let isValid: any = await this.tokens.findOne({operation: 'email-confirm', data: user.email, isExpired: false});
    if(!isValid) throw new BadRequestException({message : 'خطای اعتبار سنجی'});
    // code is not valid
    if(parseInt(isValid.token) != key){
      await this.logsService.log('email-confirm failed', user.username, user.email);
    ...
    await this.tokens.updateMany({_id : isValid._id}, {$set: {isExpired : true}});
    await this.users.findOneAndUpdate({_id : id}, {$set : {emailConfirmed : true}});
