...
export class UsersController {
  ...
  // body cant be manipulated by me
  @UseInterceptors(
  ...
  async googleRedirect(@Request() req, @Response() res){
    res.redirect(`http://localhost:4200/#/panel/registeration/login?googleAuth=${req.user}`)
  }
  ...
  async setforgotonPassword(@Body() setForgottonPasswordDto : SetForgottonPasswordDto){
    return await this.usersService.setForgottonPassword(setForgottonPasswordDto.token, setForgottonPasswordDto.password);
  }
