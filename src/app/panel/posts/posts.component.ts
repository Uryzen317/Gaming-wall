...
export class PostsComponent implements OnInit {
  ...
  uploadForm = new FormGroup({
    title: new FormControl(), // name of the selected game
  ...
  selectFile() {
    ...
    if (this.uploadForm.value.forControl && this.uploadForm.value.sizeControl) {
      fileSelection.click();
    }
    fileSelection.onchange = () => {
      if (
        ...
        this.fields.push({
          name: fileName,
          for: this.uploadForm.value.forControl,
          size: this.uploadForm.value.sizeControl
        });
      }
    };
  }
